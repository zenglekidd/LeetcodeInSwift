...
class SolutionTests: XCTestCase {
    ...
    func testExample1() {
        let input = [1,2,3,4,5,3,1]
        let target = 3
        XCTAssertEqual(solution.findInMountainArray(target, MountainArray(input)), 2)
    }

    func testExample2() {
        let input = [0,1,2,4,2,1]
        let target = 3
        XCTAssertEqual(solution.findInMountainArray(target, MountainArray(input)), -1)
    }
